...
declare global {
  interface Window {
    ...
    cmuxVoice?: {
      ...
      semanticMode: (surfaceId: string | null) => void;
    };
  }
}
...
/** The Semantic mode pill on a terminal: on for `surfaceId` (rewrite prompts there), or off (null). */
async function semanticMode(surfaceId: string | null): Promise<void> {
  if (!client) return;
  try {
    const res = (await client.sendClientRequest("semantic_mode", { surface_id: surfaceId, enabled: surfaceId != null }, 15000)) as any;
...
window.cmuxVoice = {
  ...
  semanticMode: (id: string | null) => void semanticMode(id),
};
